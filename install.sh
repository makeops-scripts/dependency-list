...
#
# Usage:
#   $ [options] curl -L bit.ly/makeops-dependency-list | bash
#
# Options:
...
INSTALL_DIR=${INSTALL_DIR:-$HOME/.makeops/dependency-list}
ORG_NAME=makeops-scripts
PROJECT_NAME=makeops-dependency-list
REPO_NAME=dependency-list
...
function check() {
  ...
  printf "Docker [%s]\n" "$value"
  which jq > /dev/null 2>&1 && value=$present || value=$missing
  printf "jq [%s]\n" "$value"
}
